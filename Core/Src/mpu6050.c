...
float Acc_Scale;
float Gyr_Scale;
float Gyr_Output_rate;
float Data_Rate;
int16_t gyro_cali_x = 0;
int16_t gyro_cali_y = 0;
int16_t gyro_cali_z = 0;
...
{
	HAL_I2C_Mem_Write(i2c, MPU6050_ADDRESS, MPU6050_RA_SMPLRT_DIV, 1, &Divider, 1, I2C_TIMEOUT);
	Data_Rate = Gyr_Output_rate / (float) (Divider + 1);
}
...
void MPU6050_SetDlpf(uint8_t Value)
{
	...
	case MPU6050_DLPF_BW_256:
		Gyr_Output_rate = 8000;
		break;
	default:
		Gyr_Output_rate = 1000;
	}
}
...
HAL_StatusTypeDef MPU6050_CalibrateGyro(void)
{
	...
	uint16_t cal_num = 2000;
	for (int i = 0; i < cal_num; ++i)
	{
		status |= MPU6050_GetAllRAW(&x, &y, &z, &rx, &ry, &rz);
		x_sum += rx;
		y_sum += ry;
		z_sum += rz;
		HAL_Delay(1);
	...
	gyro_cali_x = x_sum / cal_num;
	gyro_cali_y = y_sum / cal_num;
	gyro_cali_z = z_sum / cal_num;
	char buff[30];
	sprintf(buff, "C:x=%dy=%dz=%d\n", gyro_cali_x, gyro_cali_y, gyro_cali_z);
	send_string(buff);
	return status;
}
...
{
	uint8_t tmp[14];
	HAL_StatusTypeDef status = HAL_I2C_Mem_Read(i2c, MPU6050_ADDRESS, MPU6050_RA_ACCEL_XOUT_H, 1, tmp, 14, I2C_TIMEOUT);
	*x = (((int16_t)tmp[0]) << 8) | tmp[1];
	*y = (((int16_t)tmp[2]) << 8) | tmp[3];
	*z = (((int16_t)tmp[4]) << 8) | tmp[5];

	*rx = (((int16_t)tmp[8]) << 8) | tmp[9];
	*ry = (((int16_t)tmp[10]) << 8) | tmp[11];
	*rz = (((int16_t)tmp[12]) << 8) | tmp[13];
...
{
	int16_t tmp_x, tmp_y, tmp_z, tmp_rx, tmp_ry, tmp_rz;

	HAL_StatusTypeDef status = MPU6050_GetAllRAW(&tmp_x, &tmp_y, &tmp_z, &tmp_rx, &tmp_ry, &tmp_rz);

	result->x = (float)tmp_x * Acc_Scale;
	result->y = (float)tmp_y * Acc_Scale;
	result->z = (float)tmp_z * Acc_Scale;

	result->rx = (float)tmp_rx * Gyr_Scale;
	result->ry = (float)tmp_ry * Gyr_Scale;
	result->rz = (float)tmp_rz * Gyr_Scale;
